...
const { Detalle, Pedido, Producto } = require('../../config/database.js');
...
class DetalleController {
  ...
  static async getAllDetalles(req, res) {
    try {
      let detalles = await Detalle.findAll({
  ...
  static async createDetalle(req, res) {
    try {
      ...
      if (!cantidad || cantidad.trim() === '' || isNaN(cantidad) || parseInt(cantidad) <= 0) {
        return res.status(400).json({
          success: false,
      ...
      if (await Detalle.findOne({ where: { id_producto: id_producto} })){
        return res.status(400).json({
          success: false,
  ...
  static async updateDetalle(req, res) {
    try {
      ...
      // Validaciones básicas
      if (cantidad !== undefined && (cantidad === null || cantidad.trim() === '' || isNaN(cantidad) || parseInt(cantidad) <= 0)) {
        return res.status(400).json({
          success: false,
